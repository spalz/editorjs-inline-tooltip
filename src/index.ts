...
const addInlineStyles = () => {
  ...
  if (!document.getElementById(styleId)) {
    ...
    style.id = styleId;
    style.innerHTML = `
        .ce-inline-tooltip-tool.ce-inline-tool-input--showed {
          padding: 0;
        }
 ...
 */
export class EditorjsInlineTooltip implements EditorJS.InlineTool {
  /**
  ...
  /**
   * Native Document's commands for insertHTML and untooltip
   * @see https://stackoverflow.com/a/23891233/1238150
   */
  private readonly commandTooltip: string = "insertHTML";

  private readonly commandUntooltip: string = "untooltip";
  ...
  public render(): HTMLElement {
    ...
    this.nodes.button.innerHTML = `<svg class="icon icon--tooltip" width="14px" height="10px"><use xmlns:xlink="http://www.w3.org/1999/xlink" xlink:href="#link"></use></svg>`;
    return this.nodes.button;
  }

  /**
   * Input for the tooltip edit view
   *
   * the DOM structure for each input looks like this:
   *
   * ```
   * <div class="ce-inline-tooltip-tool">
  ...
  public renderActions(): HTMLElement {
    ...
    this.nodes.actionContainer.classList.add("ce-inline-tooltip-tool");
  ...
  public surround(range: Range): void {
    /**
     * Range will be null when user makes second click on the 'tooltip icon' to close opened input
     */
    if (range) {
      /**
  ...
  public checkState(): boolean {
    ...
    if (anchorTag) {
      ...
      // during editing tooltips. The selection will be restored later again.
      this.selection.save();
    } else {
  ...
  /**
   * @param {boolean} needFocus - on tooltip creation we need to focus input. On editing - nope.
   */
  ...
  private dataEntered(event: MouseEvent): void {
    ...
    if (!this.validateURL(value)) {
      this.notifier.show({
        message: "Pasted tooltip is not valid.",
        style: "error",
      });
      return;
    }
  ...
  /**
   * Process tooltip before injection
   * - sanitize
   * - add protocol for tooltips like 'google.com'
   * @param {string} tooltip - raw user input
   */
  private prepareLink(tooltip: string): string {
    let newLink = tooltip;
  ...
  /**
   * Add 'http' protocol to the tooltips like 'example.com', 'google.com'
   * @param {String} tooltip
   */
  private addProtocol(tooltip: string): string {
    let newLink = tooltip;
    ...
    /**
     * We need to add missed HTTP protocol to the tooltip, but skip 2 cases:
     *     1) Internal tooltips like "/general"
   ...
   * @param {string} tooltip - "href" value
   */
  private insertLink(tooltip: string): void {
    /**
     * Edit all tooltip, not selected part
     */
    const anchorTag = this.apiSelection.findParentTag("A");
    ...
    document.execCommand(
      this.commandTooltip,
      false,
      `<a href="${tooltip}" ${target} ${title}>${SelectionUtils.text}</a>`
    );
  }
   ...
   * Removes <a> tag
   */
  private untooltip(): void {
    document.execCommand(this.commandUntooltip);
  }
}
